...
public class ConsumerDemo {
	...
	public static void main(String[] args) {
		...
		String topic = "window-sink-topic";

		try (KafkaConsumer<Windowed<String>, Long> consumer = new KafkaConsumer<>(props,
				new TimeWindowedDeserializer<String>(new StringDeserializer()), new LongDeserializer());) {
			// 订阅topics
			consumer.subscribe(Arrays.asList(topic));

			while (true) {
				// kafka中是拉模式，poll的时间参数是告诉Kafka:如果当前没有数据，等待多久再响应
				ConsumerRecords<Windowed<String>, Long> records = consumer.poll(Duration.ofSeconds(1L));
